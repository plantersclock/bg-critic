...
class Top10List extends Component {
  ...
  getAuthors(items) {
    ...
    return { authors: authorChannels, authorChannels: authorChannelsDict };
  }
    ...
    if (channels.length > 0) {
      ...
      channels.map((channel) => {
        ...
        for (let i = 0; i < top10Items.length; i++) {
          if (
            top10Items[i].channel === channel &&
            !authors.includes(top10Items[i].author)
          ) {
  ...
  filterItems(items, name, value) {
    ...
    if (name === "filterPlayerCount") {
      ...
      if (filterOutChannels.includes(value)) {
        filterOutChannels = filterOutChannels.filter(
          (channel) => channel !== value
        );
        filterOutAuthors = filterOutAuthors.filter(
          (author) => !this.getChannelAuthors()[value].includes(author)
        );
      } else {
        filterOutChannels = [...filterOutChannels, value];
        filterOutAuthors = [
          ...filterOutAuthors,
          ...this.getChannelAuthors()[value],
        ];
      }
      ...
      if (filterOutAuthors.includes(value)) {
        ...
        filterOutChannels = filterOutChannels.filter(
          (channel) => channel !== authorChannels[value]
        );
      } else {
    ...
    let filteredItems = items.filter(
      ({ channel }) => !filterOutChannels.includes(channel)
    );

    filteredItems = filteredItems.filter(
      ({ author }) => !filterOutAuthors.includes(author)
    );
    if (filterPlayerCount) {
      filteredItems = filteredItems.filter(
        ({ minPlayers }) => minPlayers <= filterPlayerCount
      );
      filteredItems = filteredItems.filter(
        ({ maxPlayers }) => maxPlayers >= filterPlayerCount
      );
    }
  ...
  getTopX(items, topX) {
    ...
    return { sortedTop10Items: itemArray, topX: topX };
  }
    ...
    return (
      <div style={{ width: "100%" }}>
        <Helmet>
          ...
          <meta
            name="description"
            content={"A Dynamic Top 10 Boardgames List for " + this.state.year}
          />
        </Helmet>
        <Grid item xs={12} container justify="center" direction="row">
          ...
          >
            <Grid item xs={12} md={8}>
              ...
              ></Top10Title>
              <Top10SubText
                xReviewers={authors.length - filterOutAuthors.length}
                year={this.state.year}
              ></Top10SubText>
              {this.state.topXLoaded ? (
                structuredTop10.map((item, index) => {
            ...
            <Hidden smDown>
              <Grid item md={4}>
                <div style={{ width: "100%", height: "auto", marginTop: 38 }}>
                  ...
                  />
                  <FilterOutChannel
                    removedChannels={filterOutChannels}
                    channels={channels}
                    changeListState={this.changeListState}
                  ></FilterOutChannel>
                  <FilterOutAuthor
                    removedAuthors={filterOutAuthors}
                    authors={Object.keys(authors)}
                    authorChannels={authors}
                    changeListState={this.changeListState}
                  ></FilterOutAuthor>
